...
SELECT c.nombre_cliente, SUM(dp.precio_total) AS precio_gastado 
FROM Cliente AS c
...
FROM Producto AS pr
JOIN DetallePedido AS dp ON pr.id_producto = dp.id_producto
JOIN Pedido AS pe ON dp.id_pedido = pe.id_pedido
WHERE dp.fecha_entrega >= CURRENT_DATE - INTERVAL '1 month'
GROUP BY pr.categoria, pr.nombre_producto
ORDER BY pr.categoria, total_pedidos DESC;
-- TO DO: revisar con poblado
...
JOIN Pedido AS p ON mp.id_pedido = p.id_pedido
WHERE p.prioridad_pedido = 'Alta'
GROUP BY mp.nombre_mediodepago
ORDER BY cantidad_usos DESC
LIMIT 1;

-- TO DO: REVISAR CON DATOS